...
When dates and date-times are properly formatted in R, we can...
...
Update the "start" and "stop" columns to be Date objects using the appropriate lubridate function.
...
Period: a span of time measured in units larger than seconds, such as months. It allows us to perform calculations in calendar time as opposed to absolute number of seconds. (Eg, the subject was in the hospital 1 month.)

Let's demonstrate all three by starting with an interval. Define an interval with the `interval()` function or interval operator, `%--%`
...
Or using the interval operator:
...
Finally the `time_length()` function can be useful to specify which units to calculate for a time span.
...
1. Add a column called "duration" the measures how long the eruptive period lasted in days. Hint: first create an interval, then specify how to calculate the units.
...
```{r}
interval(msh$stop[1], ymd("2022-03-31")) |> time_length(unit = "days")
```
